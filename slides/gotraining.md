...
* ツールはたくさんある状態なので、いろいろと試してみると良いです
    ...
    * Godepが一番流行っているという印象で、GitHubのスター数も3000以上と大差がついています
    * 個人的には再帰的に依存パッケージを取ってきてくれるのでgvtが好きです
...
* チャットの特定のチャンネルの発言を全てログに出力するデーモンを作ろう
...
* チャットのあるチャンネルの発言を別のチャンネルに流すデーモンを作ろう
* 余裕のある人は下記の追加機能も実装してみてください
    * チャンネルを監視するプロセスと、チャンネルに書き込みするプロセスを分け、プロセス間のやり取りは[gRPC](http://www.grpc.io/)を使ってみよう
    * 管理プロセスを立てて、APIで監視するチャンネルを増やしたり、チャンネルに書き込みするプロセスを増やしたりしてみよう
    * この際、チャンネルに書き込みするプロセスが複数になってもメッセージの順序が保たれるようにしてください
