...
その他の言語(特にLL)を知っている人向けに、プログラミングスタイルについての注意点を書いておく。

1. 例外がないこと
2. 継承がないこと

(他には、Genericsがないのも嫌だと言う人がいるかもしれない。)

---

## 例外がないこと

golang には、 `try-catch-finally` のような構文はない。

* `panic()`, `recover()` を使って似たようなことができると思うかもしれないけど、推奨されない
    * error + defer で書く
    * `panic()`はこれ以上処理を継続できない場合にのみ利用する
        * `recover()`は使う機会はないと思って、忘れて良い
    * 参考: [Why does Go not have exceptions?](https://golang.org/doc/faq#exceptions)

--

* 例外を単に大域脱出の手段として利用したい、という主張も世の中にはあるらしい
    * できる限り関数として表現して、returnしてしまう
        * それでもどうしても大域脱出したかったら、Gotoに相当するコードをラベルと`break`で書く

---

## 継承がない

golangには継承がない。というか、オブジェクト指向によくあるクラスの概念がない。
つまり、model設計の考え方が違う。

* 構造体の埋め込みを使うと似たことができる
    * これは継承なのか?
    * 違う。これは委譲。(参考: https://play.golang.org/p/dugmSwk0Mr)

---

## 継承がない

* ざっくり言うと以下のような指針で考えれば良い(と今のところは思っている)
    * 再利用したくなる部分は、小さい構造体で実装して、構造体を埋め込む
    * クラスベースのオブジェクト指向で親クラスを指定するような部分では、インターフェースを利用する
        * もっと言うと、"オブジェクト"のようなデータ構造+メソッドに依存するのではなくて、APIに依存するように書く

たとえば、オブジェクト指向でよくやるような、動物の実装を考えてみると良い。
